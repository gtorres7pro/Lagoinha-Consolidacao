...
*   **2026-04-21:** **Unified Workspace Access GUI.** Merged Plan/Trial management and Add-on Modules into a single "Editar Workspace" modal. Centralized the list of generic and SaaS modules into `AVAILABLE_MODULES` and `PLAN_CONFIG` structures. Removed `addon_modules_config` overrides in favor of natively managing arrays directly in `workspaces.modules`. Updated `MODULE_PROTOCOL.md` to reflect granular logic implementation.
