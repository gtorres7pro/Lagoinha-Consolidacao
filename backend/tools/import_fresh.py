...
def main():
    ...
    has_more = True
    deleted_count = 0
    # Delete in batches instead of .neq due to limits
    while has_more:
        res = supabase.table("leads").select("id").limit(1000).execute()
        if not res.data:
            has_more = False
            break
    ...
    print(f"Found {len(leads_to_insert)} unique robust leads.")
